...
export const createWholeWeek = (date: Date): Date[] => {
  const weekOfTheDay = getDay(subDays(date, 1));
  const arr: Date[] = [];
  for (let i = 0; i < 7; i++) {
    ...
    if (diff === 0) {
      ...
      arr.push(subDays(date, abs));
    } else {
      arr.push(addDays(date, abs));
    }
  }
  return arr;
};
...
export const createWeekList = (locale: Locale) => {
  const list: string[] = [];
  for (let i = 1; i < 7; i++) {
    list.push(locale.localize?.day(i as Day, { width: "short" }));
  }
  list.push(locale.localize?.day(0, { width: "short" }));
  return list;
};
